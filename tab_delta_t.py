...
from aux_delta_t import get_forecasted_cumulative_delta_t
from aux_hvac import get_delta_t


def plot_delta_t(df):
    delta_t_overview(df)
    plot_forecasted_cumulative_delta_t(df)
...
def delta_t_overview(df: pd.DataFrame):
    ...
    if current_month_avg is not None:
        col2.metric(label="Month-to-Date ∆T", value=f"{current_month_avg:.2f} °C")

    if live_delta_t:
        col3.metric(label="Live ∆T", value=f"{live_delta_t:.2f} °C")
    
    if forecasted_avg is not None:
        col4.metric(label="Forecasted ∆T", value=f"{forecasted_avg:.2f} °C")
